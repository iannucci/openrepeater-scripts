...
function install_orp_config_overlay {
	...
	# Two ways for the stage to be present:
	#   (1) A pre-existing checkout at $stage (e.g. workstation rsync,
	#       useful when the repo is private and the Pi has no auth).
	#   (2) Fresh clone from GitHub HTTPS.
	# Pre-existing wins; otherwise try clone. The overlay carries
	# load-bearing data (kernel cmdline tokens, ORP_RepeaterLogic_Port1.tcl,
	# normalized WAVs, drop-ins) — without it the build looks fine but
	# the resulting card is silent and missing CPU isolation. Fail loudly
	# rather than the previous silent skip.
	if [ -d "$stage/running-system" ]; then
		echo "  using pre-staged checkout at $stage"
	else
		rm -rf "$stage"
		if ! GIT_TERMINAL_PROMPT=0 git clone --depth 1 https://github.com/iannucci/openrepeater-config.git "$stage"; then
			echo ""
			echo "*** ERROR: could not clone openrepeater-config and no pre-staged"
			echo "    checkout exists at $stage."
			echo ""
			echo "    The overlay carries the kernel cmdline isolcpus tokens,"
			echo "    ORP_RepeaterLogic_Port1.tcl (without which no audio plays),"
			echo "    the systemd drop-ins, the captured site DB, and the"
			echo "    peak-normalized WAVs. The build cannot succeed without it."
			echo ""
			echo "    Fix one of:"
			echo "      - rsync the repo's working tree to $stage before re-running"
			echo "      - make iannucci/openrepeater-config public"
			echo "      - configure git on this host with credentials for the repo"
			return 1
		fi
	fi
	local rs="$stage/running-system"
	if [ ! -d "$rs" ]; then
		echo "*** ERROR: $rs not found in staged config tree."
		return 1
	fi
	...
	# Third-party binaries, ORP TCL event handler, and systemd drop-ins.
	# /usr/share/svxlink/events.d carries ORP_RepeaterLogic_Port1.tcl —
	# without it, every TX cycle errors with "invalid command name
	# ORP_RepeaterLogic_Port1::repeater_up" and no audio plays.
	# /etc/svxlink ships the W6EI-tuned svxlink.conf, svxlink.d/* module
	# configs, and PREEMPHASIS=0. /boot/firmware is intentionally NOT in
	# this loop — see the cmdline.txt token-merge below, since a wholesale
	# copy would clobber the target card's PARTUUID with the prod card's.
	for tree in \
		/usr/local/sbin \
		/usr/local/bin \
		/usr/local/etc \
	...
	# gpio.conf is auto-generated by ORP based on the running kernel's
	# gpiochip base offsets, which differ between cards (different MCP23017
	# base on different Bookworm kernel revs, for example). The captured
	# version in the overlay is host-specific and would set wrong sysfs
	# pin numbers on any card with a different gpio_base. Force regen by
	# removing the overlaid copy; ORP regenerates it when the operator
	# clicks "Rebuild & Restart" on first boot, or when install_orp_from_github
	# stamps the initial version.
	if [ -f /etc/svxlink/gpio.conf ] && [ -f "$rs/etc/svxlink/gpio.conf" ]; then
		# Only remove if it came from the overlay (we just copied it above)
		rm -f /etc/svxlink/gpio.conf
		echo "  removed overlaid /etc/svxlink/gpio.conf (host-specific; will be regenerated by ORP)"
	fi

	# cmdline.txt: token-level merge instead of wholesale copy.
	# The captured running-system cmdline.txt has the prod card's PARTUUID,
	# which would prevent any other card from booting. Merge only the
	# OpenRepeater-relevant kernel parameters (CPU isolation: isolcpus /
	# nohz_full / rcu_nocbs) and leave the target's PARTUUID, console,
	# and rootfs config alone.
	if [ -f "$rs/boot/firmware/cmdline.txt" ] && [ -f /boot/firmware/cmdline.txt ]; then
		local cur appended=""
		cur=$(tr -d '\n' < /boot/firmware/cmdline.txt)
		for prefix in isolcpus nohz_full rcu_nocbs; do
			local src_tok
			src_tok=$(grep -oE "${prefix}=[^ ]+" "$rs/boot/firmware/cmdline.txt" | head -1)
			[ -z "$src_tok" ] && continue
			grep -qw "$src_tok" /boot/firmware/cmdline.txt && continue
			appended="$appended $src_tok"
		done
		if [ -n "$appended" ]; then
			echo "${cur}${appended}" > /boot/firmware/cmdline.txt
			echo "  merged into /boot/firmware/cmdline.txt:$appended"
		fi
	fi
